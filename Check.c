...
void* write_test(void* vargp) {
    config* conf = (config*)vargp;
    uint64_t ret = 0;
    int tid = conf->tid;
    long long portion = conf->size / conf->tnum;
    long long start = portion + conf->blk_size
    ...
    for (int i = 0; i < conf->round; i++) {
        ...
        ret += PWRITE(conf->fd, conf->buffer, conf->blk_size, conf->rnd_addrs[i]);
    }
...
int main(int argc, char ** argv){
    ...
    long long start = nrand48(seeds) % (size - rnd_blk_size + 1);
    ...
    for (int i = 0; i < num_thread; i++) {
        ...
        pthread_create(&threads[i], NULL, write_test, (void*)&conf[i]);
    }
    for (int i = 0; i < num_thread; i++)
    {
    ...
    printf(" Write Test Completed with %d Bytes Block: \n", rnd_blk_size);
    ...
    // READ THE BUFF, check if all data is consistence in the memory
    fd = OPEN(path, O_RDONLY | O_SYNC, S_IRWXU);
    rnd_buf = malloc(conf->blk_size);
    pread(fd, rnd_buf, rnd_blk_size, rnd_addrs[0]);
    int target = rnd_buff[0];
    for (uint64_t j = 0; j < (rnd_blk_size / sizeof(char)); j++) {
        if ((int)rnd_buf[j] != target) {
